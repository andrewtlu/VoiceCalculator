...
/* Code based off of:
 * https://github.com/goxr3plus/Java-Text-To-Speech-Tutorial/blob/master/Mary%20TTS%20Program/src/application/AudioPlayer.java
 *
 * Copyright 2017 goxr3plus
 *
 * Original code commenting changed
 */
...
public class AudioPlayer extends Thread {
    ...
    public void setAudio(AudioInputStream audio) {
        if (status == Status.PLAYING) throw new IllegalStateException("Cannot set audio while playing");
        this.ais = audio;
    }

    /** Cancel and exit AudioPlayer thread */
    public void cancel() {
    ...
    /** Sets Gain value, assumes line is open, value between 0.0 and 1.0, use 0.5 to avoid saturation */
    ...
    public void run() {
        ...
        if (audioFormat.getChannels() == 1) {
            ...
            if (outputMode == 0) ais = new MonoAudioInputStream(ais);
            else if (outputMode == 1 || outputMode == 2) ais = new StereoAudioInputStream(ais, outputMode);
            else assert outputMode == 3 : "Unexpected output mode: " + outputMode;
        }
